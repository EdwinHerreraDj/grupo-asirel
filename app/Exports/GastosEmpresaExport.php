...
class GastosEmpresaExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        return GastoGeneralEmpresa::with('categoria')
            ->when($this->inicio, fn($q) => $q->whereDate('fecha_factura', '>=', $this->inicio))
            ->when($this->fin, fn($q) => $q->whereDate('fecha_factura', '<=', $this->fin))
            ->orderByDesc('fecha_factura')
            ->get()
            ->map(fn($gasto) => [
                'Concepto' => $gasto->concepto,
                'Categoría' => $gasto->categoria->nombre ?? '—',
                'Fecha' => $gasto->fecha_factura
                    ? Carbon::parse($gasto->fecha_factura)->format('d/m/Y')
                    : '',
