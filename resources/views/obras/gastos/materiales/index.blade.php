@extends('layouts.vertical', ['title' => 'Gastos Materiales', 'sub_title' => 'Pages', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
        ...
        <div class="col-span-12">
            ...
            <div class="card p-10">
               ...
               shadow-sm hover:bg-green-600 hover:text-white transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-400/40">
                ...
                {{-- Modal para agregar gasto de materiales --}}
                @include('obras.gastos.materiales.modal.create')
                ...
                @include('obras.gastos.materiales.modal.informe')



                {{-- Tabla de Gastos materiales --}}
                            ...
                            <tbody>
                                @foreach ($obra->materiales as $material)
                                    <tr>
                                        ...
                                        <td>{{ number_format($material->importe, 2, ',', '.') }} €</td>
                                        <td>
                                            <x-btns.descargar href="{{ asset('storage/' . $material->archivo_factura) }}" />
                                            <x-btns.ver pdf="{{ asset('storage/' . $material->archivo_factura) }}" />
                                        </td>
                                        <td>{{ $material->numero_factura }}</td>
                                        <td>{{ \Carbon\Carbon::parse($material->fecha)->format('d/m/Y') }}</td>

                                        <td>
                                            <x-btns.eliminar class="delete-registro" id="{{ $material->id }}" />
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
