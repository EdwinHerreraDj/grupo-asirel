...
<body>
    ...
    {{-- TABLA DE ALQUILERES / MATERIALES --}}
    <table>
        <thead>
            <tr>
                <th>Material</th>
        ...
        <tbody>
            ...
            @foreach ($alquileres as $alquiler)
                @php $subtotal = $alquiler->cantidad * $alquiler->precio_unitario; @endphp
                <tr>
                    <td>{{ $alquiler->nombre }}</td>
                    <td>{{ $alquiler->descripcion }}</td>
                    <td>
                        {{ $alquiler->fecha ? \Carbon\Carbon::parse($alquiler->fecha)->format('d/m/Y') : 'Sin fecha' }}
                    </td>
                    <td>{{ $alquiler->numero_factura }}</td>
                    <td>{{ $alquiler->cantidad }}</td>
                    <td>{{ number_format($alquiler->precio_unitario, 2, ',', '.') }} €</td>
